$(document).ready(function () {
    let hour = parseInt((durationTime / (1000 * 60 * 60)) % 24);
    let minute = parseInt((durationTime / (1000 * 60)) % 60);
    ...
    $('#duration-hours').val(hour);
    $('#duration-minutes').val(minute);
})
...
$(document).ready(function () {
    ...
    $(document).on('click', '#changeMovieBtn', function () {
        ...
        if ($('#name').val().length === 0 || $('#description').val().length === 0 ||
            ...
            $.ajax({
                url: contextPath + '/admin/change/movie',
                type: 'POST',
                dataType: 'json',
