...
//    }
    ...
    @Order(2)
    void findById() {
        Seance seance = sr.findById(1L).get();
//        seance.setMovieBeginTime(Time.valueOf(seance.getMovieBeginTime().toLocalTime().minus(3,
//                ChronoUnit.HOURS)));
//        System.out.println(seance.getMovieBeginTime());
//        List<Seance> seance1 =
//                sr.findByTimeLineInSeance(seance.getMovieBeginTime()).stream().filter(s -> s.getMovie() == seance.getMovie()).collect(Collectors.toList());
//        System.out.println(seance1.get(0).getMovieBeginTime());
    }
